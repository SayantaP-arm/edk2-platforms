...
/**
  ...
  to indicate a successful read, a DWORD at a time until the entire DOE Response
...
/**
  ...
  for number of memory nodes is more than maximum remote memory nodes, then
...
CxlInstallProtocol (VOID)
{
  ...
  CxlPlatformProtocol = (CXL_PLATFORM_PROTOCOL *) AllocateZeroPool (
                                                    sizeof(CXL_PLATFORM_PROTOCOL)
                                                    );
